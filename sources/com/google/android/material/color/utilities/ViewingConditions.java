...
public final class ViewingConditions {
    ...
    public static ViewingConditions make(double[] whitePoint, double adaptingLuminance, double backgroundLstar, double surround, boolean discountingIlluminant) {
        ...
        double d5 = nbb2;
        double ncb2 = nbb2;
        double[] rgbAFactors = {Math.pow(((rgbD3[0] * fl2) * rW) / 100.0d, 0.42d), Math.pow(((rgbD3[1] * fl2) * gW) / 100.0d, 0.42d), Math.pow(((rgbD3[2] * fl2) * bW) / 100.0d, 0.42d)};
        ...
        double[] dArr = rgbAFactors;
        double[] dArr2 = rgbA;
        return new ViewingConditions(d4, ((rgbA[0] * 2.0d) + rgbA[1] + (rgbA[2] * 0.05d)) * nbb2, d5, ncb2, c2, f, rgbD2, fl2, Math.pow(fl2, 0.25d), Math.sqrt(n2) + 1.48d);
    }
